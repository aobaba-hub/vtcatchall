...
class Player(Tk.Frame):
    ...
    def __init__(self, parent, title=None, video='vt-player', border=10):
        ...
        fileMenu.add_command(label="Stops", command=self.OnStop)
        fileMenu.add_separator()
        ...
        self.playButton = ttk.Button(buttons, text="pPlay", command=self.OnPlay)
        stop            = ttk.Button(buttons, text="pStop", command=self.OnStop)
        self.muteButton = ttk.Button(buttons, text="mMute", command=self.OnMute)
        ...
        timers = ttk.Frame(self.buttons_panel, border=5)
    ...
    def OnPlay(self, *unused):
        ...
        contents = ["C:/Temp/title_t02.mkv", "C:/Temp/title_t01.mkv", "C:/Temp/title_t00.mkv"]
        # for i in range(len(contents)):
        #     video = contents[i].strip()
        #     print("playing this video: ", video)
        media = vlc.Media("C:/Temp/title_t01.mkv")
        self.player.set_media(media)


        # mediaList = vlc.MediaList()
        # mediaList.add_media(media)
        # newlist = vlc.libvlc_media_list_new(self)
        ...
        # Try to play, if this fails display an error message
            ...
            print("now playing........ for: ", mm)
            while True:
                state = self.player.get_state()
                print("current state: ", state)
                if state in vlc.State:
                    print("movie ended: ", mm)
                    break
                else:
                    print("now playing........ for: ", mm)
                time.sleep(1)
